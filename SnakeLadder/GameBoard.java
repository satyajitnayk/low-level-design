...
public class GameBoard {
  ...
  GameBoard(Dice dice, Queue<Player> nextTurn, List<Jumper> snakes,
      List<Jumper> ladders, Map<String, Integer> playersCurrentPosition, int boardSize) {
    this.dice = dice;
  ...
  void startGame() {
    ...
    while (nextTurn.size() > 1) {
      ...
      // Publish the players name whose turn is it
      ...
      if (nextCell > boardSize)
        ...
        if (nextPosition[0] != nextCell && b[0]) {
          System.out.println(player.getPlayerName() + " got a ladder at: " + nextCell);
        }

        if (nextPosition[0] == boardSize) {
          System.out.println(player.getPlayerName() + " won the game");
        } else {
          playersCurrentPosition.put((String) player.getPlayerName(), nextPosition[0]);
